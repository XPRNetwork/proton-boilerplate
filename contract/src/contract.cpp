...
namespace proton
{
  void contract::addplan (const Plan& plan) {
  ...
  void contract::updateplan (const Plan& plan) {
    require_auth(get_self());
  ...
  void contract::removeplan (const uint64_t& plan_index) {
    require_auth(get_self());
  ...
  void contract::process (const uint64_t& max) {
  ...
  void contract::end_term(const Term& term) {
