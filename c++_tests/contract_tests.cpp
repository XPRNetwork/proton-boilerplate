...
public:
   proton_tester() {
      ...
      // Deploy user contract (contract)
      set_code( N(contract), contracts::atom_wasm() );
      set_abi( N(contract), contracts::atom_abi().data() );
      produce_blocks();

      // Add contract ABI to ABI serializer
      const auto& wasm_accnt = control->db().get<account_object, by_name>( N(contract) );
      abi_def wasm_abi;
...
BOOST_AUTO_TEST_SUITE(proton_test)
   BOOST_FIXTURE_TEST_CASE( test_case1, proton_tester ) try {
      ...
      transfer(N(proton), N(contract), asset::from_string("200.0000 XPR"), "Hello");
