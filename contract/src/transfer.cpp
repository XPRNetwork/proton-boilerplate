...
namespace proton {
  void contract::ontransfer (const eosio::name& from, const eosio::name& to, const eosio::asset& quantity, const std::string& memo) {
    // Process action
    process(10);
  ...
  void contract::withdraw (const eosio::name& account, const eosio::extended_asset& balance) {
    require_auth(account);
    substract_balance(account, balance);
    transfer_to(account, balance, "");
  }

  void contract::transfer_to(const eosio::name& to, const eosio::extended_asset& balance, const std::string& memo) {
